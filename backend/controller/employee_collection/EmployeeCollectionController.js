...
const GetDeliveryData = async (req, res) => {
  try {
    ...
    const sql2 =
      "SELECT collectionName, collectionId FROM collections WHERE collectionId <> :employee_place";
...
const GetDeliveryFromCollecton = async (req, res) => {
  try {
    const sql1 =
      "SELECT DISTINCT transactions.transactionId as transactionId, transactions.status as status, transactions.distriReceived as distributionId, sender.customerName as senderName, sender.address as senderAddress, sender.numberPhone as senderPhone," +
      " receiver.customerName as receiverName, receiver.address as receiverAddress, receiver.numberPhone as receiverPhone," +
      " parcels.width as parcelWidth, parcels.height as parcelHeight, parcels.length as parcelLength, parcels.parcelContent as parcelContent, parcels.parcelType as parcelType, parcels.weight as parcelWeight," +
      " journey1.status as journeyStatus," +
      " fees.feeMain as feeMain, fees.feeSub as feeSub, fees.cod as cod, fees.vat as vat, fees.gtgt as gtgt, fees.other as other, fees.total as total FROM transactions" +
    ...
    const sql2 =
      "SELECT distributionName, distributionId FROM distributions WHERE collectionId = :employee_place";
    const data2 = await database.query(sql2, {
      replacements: {
        employee_place: req.Workplace,
      },
    ...
    return res
      .status(200)
      .json({ deliveries: data1[0], distributions: data2[0] });
  } catch (error) {
    return res.status(400).json({ msg: error.message });
  }
};
...
const UpdateJourney = async (req, res) => {
  try {
    ...
    if (req.body.status === "2") {
      ...
      await Transaction.update(
        {
          distriReceived: req.body.distributionId,
        },
        {
          where: {
            transactionId: req.params.id,
      ...
      await Journey.update(
        {
          status: "6",
          sendFromCollection2At: new Date(),
        },
        {
          where: {
            journeyId: transaction.journeyId1,
